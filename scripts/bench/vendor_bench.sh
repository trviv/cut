...
if [[ $QUICK -eq 1 ]]; then
  if [[ -n "$FILTER" ]]; then
    echo "Error: --quick and --filter cannot be used together" >&2
    exit 1
  fi
  FILTER="-sort_radix/N=(4194304|16777216)"
fi
...
if [[ $DO_BUILD -eq 1 ]]; then
  ...
  for target in cublas_matmul_bench cublas_extras_bench cudnn_softmax_bench cub_scan_sort_bench rocblas_matmul_bench rocprim_scan_sort_bench; do
