...
class Buffer(BaseBuffer):
    ...
    def __init__(self, data: Optional[np.ndarray] = None, size: Optional[int] = None,
        ...
        if data is not None:
            ...
            self._init_empty(_interface, _cut_cpu, size, dtype, shape, is_uniform)
        else:
...
class Dispatch(BaseDispatch):
    ...
    def __init__(self, kernel: Kernel, thread_groups: tuple = (1, 1, 1)):
        ...
        """
        dispatch_obj = _cut_cpu.ComputeDispatch(kernel.handle)
        super().__init__(dispatch_obj, _cut_cpu.ThreadSize)
