// Native CUDA counterpart of BinaryFusedVecScalarVecVec.shader (vector-scalar-vector-vector fused binary op).
...
struct PushConstants {
    uint numElements;
    CUT_SCALAR_DTYPE_INPUT1 scalar;
};
...
extern "C" __global__ void cut_main(const cut_in1_vec* __restrict__ dataA,
    ...
    cut_out_vec s = CUT_BIN_OUT_CAST(CUT_BIN_IN1_CAST(cut_bin_widen(pc.scalar)));
