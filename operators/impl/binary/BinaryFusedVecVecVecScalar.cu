// Native CUDA counterpart of BinaryFusedVecVecVecScalar.shader (vector-vector-vector-scalar fused binary op).
...
struct PushConstants {
    uint numElements;
    CUT_SCALAR_DTYPE_OUTPUT scalar;
};
...
extern "C" __global__ void cut_main(const cut_in1_vec* __restrict__ dataA,
    ...
    cut_out_vec s = CUT_BIN_OUT_CAST(cut_bin_widen(pc.scalar));
    cut_out_vec intermediate = cut_binary_apply(op_enum1, a, b);
    dataOut[index] = cut_binary_apply(op_enum2, intermediate, s);
}
