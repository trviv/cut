// Native CUDA counterpart of TernaryClamp.shader. Dtype selection via NVRTC -D defines.
...
struct PushConstants {
    uint numElements;
    CUT_SCALAR_DTYPE_INPUT minVal;
    CUT_SCALAR_DTYPE_INPUT maxVal;
};

extern "C" __global__ void cut_main(const CUT_VEC_DTYPE_INPUT* __restrict__ dataIn, CUT_VEC_DTYPE_INPUT* __restrict__ dataOut, PushConstants pc) {
    uint3 DTid;
    ...
    dataOut[index] = clamp(dataIn[index], CUT_BCAST_INPUT(pc.minVal), CUT_BCAST_INPUT(pc.maxVal));
}
