...
// Helper: map QuantFormat to the SiLU OperatorEnum
static OperatorEnum siluEnum(QuantFormat fmt) {
  switch (fmt) {
  case QuantFormat::Q8:
    return MatMulQ8SiLU;
  case QuantFormat::Q4:
...
struct SiLUVariantInfo {
  uint32_t wgX, wgY, effTileM, effTileN;
};

static SiLUVariantInfo getSiLUVariant(QuantFormat fmt, uint32_t spec) {
  switch (fmt) {
  case QuantFormat::Q8: {
...
MatMulSiLUOpNode::MatMulSiLUOpNode(TensorStore &store,
    ...
    : OpNode(MatMulSiLU, store, spec), format_(QuantFormat::None) {
...
MatMulSiLUOpNode::MatMulSiLUOpNode(TensorStore &store,
  ...
  dtypeA_ = bufA.getDtype();
  dtypeB_ = bufScalesB.getDtype();

  if (shapePB.size() != 2 || shapeSB.size() != 2) {
    throw std::runtime_error("matmulSiLU requires 2D B and scales matrices");
  }
  if (shapeA.size() == 1) {
    M_ = 1;
    K_ = shapeA[0];
    ...
    throw std::runtime_error("matmulSiLU: A must be 1D or 2D");
  }

  if (shapePB[0] != K_) {
    throw std::runtime_error(
        "matmulSiLU: B rows (" + std::to_string(shapePB[0]) +
  ...
  if (bCols == sCols) {
    ...
    throw std::runtime_error(
        "matmulSiLU: cannot determine quant format from B cols (" +
        std::to_string(bCols) + ") and scales cols (" + std::to_string(sCols) +
        ")");
  }

  op_ = siluEnum(format_);

  constexpr uint32_t kSiLUTiledVariant = 1;
  spec_ = spec.value_or(kSiLUTiledVariant);
...
ThreadSize MatMulSiLUOpNode::dispatchSize() const {
  auto vi = getSiLUVariant(format_, *spec_);
...
std::string MatMulSiLUOpNode::displayName() const {
  switch (format_) {
  case QuantFormat::Q8:
    return "MatMulQ8SiLU";
  case QuantFormat::Q4:
