...
// Fused Q8_0 dequant matmul with binary operation: binaryOp(A * dequant(B), D)
...
#include "MatMulQ8BinaryCommon.shaderh"
...
void main(uint3 GTid : SV_GroupThreadID, uint3 Gid : SV_GroupID) {
    ...
    if (fullM && fullN) {
        ...
        // Unchecked output write with binary operation
        [unroll] for (uint m = 0; m < TM; m++) {
            [unroll] for (uint n = 0; n < TN; n++) {
                uint outRow = blockRowStart + localRow + m * TILE_SIZE;
                uint outCol = blockColStart + localCol + n * TILE_SIZE;
                dataC[outRow * pc.strideC + outCol] = applyBinaryOp(acc[m][n], loadD(outRow, outCol));
            }
        }
    } else {
        ...
        // Checked output write with binary operation
        [unroll] for (uint m = 0; m < TM; m++) {
            [unroll] for (uint n = 0; n < TN; n++) {
                uint outRow = blockRowStart + localRow + m * TILE_SIZE;
                uint outCol = blockColStart + localCol + n * TILE_SIZE;
