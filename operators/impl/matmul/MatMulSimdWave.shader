...
void main(uint3 GTid : SV_GroupThreadID, uint3 Gid : SV_GroupID) {
    ...
    for (uint kt = 0; kt < numKTiles; kt++) {
        ...
        %SCALAR_DTYPE_INPUT2% b_reg[TN];
        [unroll] for (uint n = 0; n < TN; n++) {
            uint bRow = kBase + wr;
            uint bCol = blockColStart + wc * TN + n;
            b_reg[n] = (wr < WC) ? loadB(bRow, bCol) : (%SCALAR_DTYPE_INPUT2%)(0);
        }
        ...
        [unroll] for (uint kk = 0; kk < WC; kk++) {
            ...
            [unroll] for (uint m = 0; m < TM; m++)
                [unroll] for (uint n = 0; n < TN; n++)
                    acc[m][n] += (%SCALAR_DTYPE_OUTPUT%)(a_k[m]) * (%SCALAR_DTYPE_OUTPUT%)(b_k[n]);
        }
    }
