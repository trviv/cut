// Native CUDA counterpart of MatMulTiledRegBReg.shader (tiled matmul, B-register pre-load).
    ...
    if (fullM && fullN) {
        for (uint t = 0; t < fullKTiles; t++) {
            ...
            for (uint k = 0; k < TILE_SIZE; k += 4) {
                ...
                #pragma unroll
                for (uint m = 0; m < TM; m++) {
                    cut_a_t a0 = (cut_a_t)tileA[localRow + m * TILE_SIZE][k];
                    cut_a_t a1 = (cut_a_t)tileA[localRow + m * TILE_SIZE][k + 1];
                    cut_a_t a2 = (cut_a_t)tileA[localRow + m * TILE_SIZE][k + 2];
                    cut_a_t a3 = (cut_a_t)tileA[localRow + m * TILE_SIZE][k + 3];
                    #pragma unroll
                    for (uint n = 0; n < TN; n++) {
