// Native CUDA counterpart of MatMulTiledRegVec.shader (tiled matmul, vec4 cooperative loads).
...
extern "C" __global__ void cut_main(const cut_a_vec* __restrict__ dataA, const cut_b_vec* __restrict__ dataB, const cut_d_vec* __restrict__ dataD, cut_c_t* __restrict__ dataC, PushConstants pc) {
    ...
    uint2 tileId = mmSwizzle(pc, TILE_SIZE * TM, TILE_SIZE * TN, blockIdx.x, blockIdx.y);
    ...
    uint tileA_v4 = tid % (TILE_SIZE / 4);
    uint tileA_col = tileA_v4 * 4;

    uint tileB_row = tid / (TILE_SIZE * TN / 4);
    uint tileB_v4 = tid % (TILE_SIZE * TN / 4);
    uint tileB_col = tileB_v4 * 4;
