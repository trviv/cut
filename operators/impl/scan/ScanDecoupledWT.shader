// Vulkan counterpart of ScanDecoupledWT.cu — NEVER DISPATCHED. The WT family is
// CUDA-only (ScanOp.h::scanVariantIsCudaOnly): its subject is a warp-transpose
// through shared between the load and the scan, and this file does not
// implement it. It exists because native .cu kernels are keyed by the
// NORMALIZED SPIR-V HASH of their .shader counterpart, so each variant needs
// one, and VARIANT_TAG below is what stops WTIPT<n> from hashing identically to
// ScanIPT<n> and silently running ScanDecoupled.cu instead. Gate on
// "WARNING: native hash collision" in the configure log.
//
// Body is ScanDecoupled.shader verbatim; see it for the protocol, and
// ScanDecoupledWT.cu for what the variants actually do.
...
// SPIR-V identity for a CUDA-only knob. Must reach code the compiler cannot
// fold away — an unused #define changes nothing about the SPIR-V. numElements
// is a dispatch-derived count and can never be UINT_MAX, so this never runs.
#define VARIANT_TAG (%IPT% * 7u + 3u)
...
void main(uint3 GTid : SV_GroupThreadID) {
    ...
    // Fold and stage back through shared so the global stores stay coalesced
    // (striped) — but fold ONLY the per-thread part of the prefix here.
    //
    // This runs BEFORE the look-back, which is what removes a barrier: the
    // look-back's own GroupMemoryBarrierWithGroupSync() then doubles as the one
    // ordering these writes against the striped reads at the bottom. The split is
    // what makes it legal. threadPrefix belongs to THIS thread's slice and can
    // only be applied while the data is still blocked — after the transpose a
    // thread reads slots owned by other threads, whose threadPrefix differs. The
    // tile-wide exclusive prefix is block-uniform, so it is the one term that may
    // be added on the far side of the transpose. Mirrors ScanDecoupled.cu.
    [unroll]
    for (uint r = 0; r < IPT; r++) {
        scalar_t outv = (pc.isExclusive != 0u)
                            ? ((r == 0u) ? (scalar_t)0 : v[r - 1])
                            : v[r];
        sData[tid * IPT + r] = threadPrefix + outv;
    }
    ...
    // This barrier does double duty: it publishes sExclusive AND orders the fold's
    // writes above against the striped reads below.
    GroupMemoryBarrierWithGroupSync();
    ...
    for (uint r = 0; r < IPT; r++) {
        ...
        uint g = tileStart + i;
        if (g < pc.numElements) {
            dataOut[g] = sData[i] + tileExclusive;
        }
    }

    if (pc.numElements == 0xFFFFFFFFu) {
        dataOut[0] = (scalar_t)VARIANT_TAG;
    }
