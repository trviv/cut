// Native CUDA counterpart of LogSoftmax.shader — keep semantics in lockstep.
// Fused single-pass log-softmax: one 256-thread block per slice.
...
extern "C" __global__ void cut_main(const CUT_SCALAR_DTYPE_INPUT* __restrict__ dataIn,
    ...
    // Phase 3: write log-softmax output = (x - max) - log(sumexp)
    float log_sumexp = logf(global_sumexp);
    for (uint r = tid; r < pc.reduceSize; r += WG_SIZE) {
        uint idx = outer * pc.inOuterStride + r * pc.inReduceStride + inner;
        float x = (float)(dataIn[idx]);
        dataOut[idx] = (CUT_SCALAR_DTYPE_INPUT)(x - global_max - log_sumexp);
    }
}
