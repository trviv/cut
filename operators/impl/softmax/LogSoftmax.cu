// Native CUDA log-softmax. The algorithm — warp-or-block per row, vectorized,
// register-resident when the row fits — lives in SoftmaxCommon.cuh, which also
// documents how and why it diverges from LogSoftmax.shader.
...
extern "C" __global__ void cut_main(const CUT_SCALAR_DTYPE_INPUT* __restrict__ dataIn,
                                    ...
                                    PushConstants pc) {
    cut_softmax_kernel<true>(dataIn, dataOut, pc);
}
