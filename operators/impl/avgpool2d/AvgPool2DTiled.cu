// Native CUDA counterpart of AvgPool2DTiled.shader (block dims TILE_W x TILE_H
// x 1; shared-memory input tile + halo with OOB sentinel) — keep semantics in
// lockstep.
#include "AvgPool2DCommon.cuh"

#ifndef TILE_W
#define TILE_W 16
#endif
#ifndef TILE_H
#define TILE_H 16
#endif
#define MAX_KH 11
#define MAX_KW 11
#define SHARED_H (TILE_H * 2 + MAX_KH - 1)
#define SHARED_W (TILE_W * 2 + MAX_KW - 1)

// Sentinel value to mark out-of-bounds positions
#ifdef CUT_DTYPE_INPUT_IS_FLOAT
#define OOB_SENTINEL (-1.0e38)
#else
#define OOB_SENTINEL (-2147483648)
#endif

extern "C" __global__ void cut_main(const cut_pool_t* __restrict__ input_data,
                                    cut_pool_t* __restrict__ output_data,
                                    PushConstants pc) {
    __shared__ cut_pool_t sharedInput[SHARED_H][SHARED_W];

    uint H_out = (pc.H_in + 2 * pc.padH - pc.kernelH) / pc.strideH + 1;
    uint W_out = (pc.W_in + 2 * pc.padW - pc.kernelW) / pc.strideW + 1;
    uint inAlignedW = (pc.W_in + 3) & ~3u;
    uint outAlignedW = (W_out + 3) & ~3u;

    // z encodes (n, c), x/y are spatial
    uint dz = blockIdx.z * blockDim.z + threadIdx.z;
    uint c = dz % pc.C;
    uint n = dz / pc.C;
    uint h_out = blockIdx.y * TILE_H + threadIdx.y;
    uint w_out = blockIdx.x * TILE_W + threadIdx.x;

    bool active = (w_out < W_out && h_out < H_out);

    // Actual shared memory region needed for this tile
    uint sharedH = (TILE_H - 1) * pc.strideH + pc.kernelH;
    uint sharedW = (TILE_W - 1) * pc.strideW + pc.kernelW;

    int baseH = int(blockIdx.y * TILE_H * pc.strideH) - int(pc.padH);
    int baseW = int(blockIdx.x * TILE_W * pc.strideW) - int(pc.padW);

    // Cooperatively load input tile + halo into shared memory
    for (uint sh = threadIdx.y; sh < sharedH; sh += TILE_H) {
        for (uint sw = threadIdx.x; sw < sharedW; sw += TILE_W) {
            int ih = baseH + int(sh);
            int iw = baseW + int(sw);
            cut_pool_t val = (cut_pool_t)(OOB_SENTINEL);
            if (ih >= 0 && ih < int(pc.H_in) && iw >= 0 && iw < int(pc.W_in)) {
                uint in_idx = n * pc.C * pc.H_in * inAlignedW
                            + c * pc.H_in * inAlignedW
                            + uint(ih) * inAlignedW
                            + uint(iw);
                val = input_data[in_idx];
            }
            if (sh < SHARED_H && sw < SHARED_W) {
                sharedInput[sh][sw] = val;
            }
        }
    }
    __syncthreads();

    if (!active) return;

    // Compute average pooling from shared memory
    uint localH = threadIdx.y * pc.strideH;
    uint localW = threadIdx.x * pc.strideW;

    cut_pool_t sum = (cut_pool_t)(0);
    uint count = 0;

    for (uint kh = 0; kh < pc.kernelH; kh++) {
        for (uint kw = 0; kw < pc.kernelW; kw++) {
            cut_pool_t val = sharedInput[localH + kh][localW + kw];
            if (val != (cut_pool_t)(OOB_SENTINEL)) {
                sum += val;
                count++;
            }
        }
    }

    cut_pool_t result = (cut_pool_t)(0);
    if (count > 0) {
        result = sum / (cut_pool_t)(count);
    }

    uint out_idx = n * pc.C * H_out * outAlignedW
                 + c * H_out * outAlignedW
                 + h_out * outAlignedW
                 + w_out;

    output_data[out_idx] = result;
}
