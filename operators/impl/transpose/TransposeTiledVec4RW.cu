// Native CUDA counterpart of TransposeTiledVec4RW.shader — keep semantics in lockstep.
//
// Vec4 read + Vec4 write tiled transpose. Uses vec4 loads for input and vec4
// stores for output, reducing memory transactions by 4x on both sides.
...
extern "C" __global__ void cut_main(const CUT_VEC_DTYPE_INPUT* __restrict__ dataIn,
    ...
    CUT_VEC_DTYPE_INPUT v;
    v[0] = (CUT_SCALAR_DTYPE_INPUT)(0);
    v[1] = (CUT_SCALAR_DTYPE_INPUT)(0);
    v[2] = (CUT_SCALAR_DTYPE_INPUT)(0);
    v[3] = (CUT_SCALAR_DTYPE_INPUT)(0);
    ...
    if (baseCol + 0 >= pc.N) v[0] = (CUT_SCALAR_DTYPE_INPUT)(0);
    if (baseCol + 1 >= pc.N) v[1] = (CUT_SCALAR_DTYPE_INPUT)(0);
    if (baseCol + 2 >= pc.N) v[2] = (CUT_SCALAR_DTYPE_INPUT)(0);
    if (baseCol + 3 >= pc.N) v[3] = (CUT_SCALAR_DTYPE_INPUT)(0);
    ...
    // Remap 256 threads to cover 64 output rows x 4 vec4 groups
