...
private:
  ...
  std::string readString(const std::vector<uint32_t> &instruction,
                         size_t startIndex) {
    std::string result;
  ...
  std::string storageClassToString(SpvStorageClass sc) {
    ...
    case SpvStorageClass::Output:
      return "Output";
    case SpvStorageClass::Workgroup:
      return "Workgroup";
    case SpvStorageClass::CrossWorkgroup:
      return "CrossWorkgroup";
    case SpvStorageClass::Private:
      return "Private";
    case SpvStorageClass::Function:
      return "Function";
    case SpvStorageClass::Generic:
      return "Generic";
    case SpvStorageClass::PushConstant:
      return "PushConstant";
    case SpvStorageClass::AtomicCounter:
      return "AtomicCounter";
    case SpvStorageClass::Image:
      return "Image";
  ...
  std::string decorationToString(SpvDecoration dec) {
    switch (dec) {
    case SpvDecoration::Block:
      return "Block";
    case SpvDecoration::BufferBlock:
      return "BufferBlock";
    case SpvDecoration::RowMajor:
      return "RowMajor";
    case SpvDecoration::ColMajor:
      return "ColMajor";
    case SpvDecoration::ArrayStride:
      return "ArrayStride";
    case SpvDecoration::MatrixStride:
      return "MatrixStride";
    ...
    case SpvDecoration::Offset:
      return "Offset";
    case SpvDecoration::NonReadable:
      return "NonReadable";
    case SpvDecoration::NonWritable:
      return "NonWritable";
    case SpvDecoration::Restrict:
      return "Restrict";
    case SpvDecoration::Volatile:
      return "Volatile";
    case SpvDecoration::Coherent:
      return "Coherent";
    default:
      return "Decoration" + std::to_string(static_cast<uint32_t>(dec));
    }
  }
...
public:
  bool readSpirvBinary(const std::string &filepath) {
    ...
    if (magic != SPIRV_MAGIC) {
      std::cerr << "Error: Invalid SPIR-V magic number: 0x" << std::hex << magic
                << std::endl;
      return false;
    }
    ...
    std::cout << "SPIR-V Version: " << (version >> 16) << "."
              ...
              << std::endl;
    std::cout << "Generator: 0x" << std::hex << generator << std::dec
              << std::endl;
    ...
    while (offset < fileSize) {
      ...
      if (length == 0 || offset + length * 4 > fileSize)
        break;

      std::vector<uint32_t> instruction;
      for (int i = 0; i < length; ++i) {
        uint32_t word =
            *reinterpret_cast<const uint32_t *>(data.data() + offset + i * 4);
  ...
  void analyzeInstructions() {
    ...
    for (const auto &instruction : instructions) {
      ...
      case SpvOpcode::OpDecorate:
        if (instruction.size() >= 3) {
          ...
          std::vector<uint32_t> params(instruction.begin() + 3,
                                       instruction.end());
      ...
      case SpvOpcode::OpTypePointer:
        if (instruction.size() >= 4) {
          ...
          types[resultId] = TypeInfo(
              "Pointer<" +
              storageClassToString(static_cast<SpvStorageClass>(storageClass)) +
              ">");
        }
        break;
      ...
      case SpvOpcode::OpTypeStruct:
        if (instruction.size() >= 2) {
          uint32_t resultId = instruction[1];
          TypeInfo typeInfo("Struct(" + std::to_string(instruction.size() - 2) +
                            " members)");
          typeInfo.member_types.assign(instruction.begin() + 2,
                                       instruction.end());
          types[resultId] = typeInfo;
        }
        break;
      ...
      case SpvOpcode::OpTypeInt:
        if (instruction.size() >= 4) {
          ...
          types[resultId] =
              TypeInfo((signedness ? "int" : "uint") + std::to_string(width));
        }
        break;
    ...
    for (const auto &instruction : instructions) {
      ...
      if (opcode == SpvOpcode::OpVariable && instruction.size() >= 4) {
        ...
        resource.name = names.count(resultId)
                            ? names[resultId]
                            : ("unnamed_" + std::to_string(resultId));
        resource.type =
            types.count(resultType) ? types[resultType].name : "unknown";
  ...
  void printResources() {
    ...
    auto printResourceGroup = [this](const std::string &title,
      ...
      std::sort(resources.begin(), resources.end(),
                [](const ResourceInfo &a, const ResourceInfo &b) {
                  if (a.descriptor_set.has_value() !=
                      b.descriptor_set.has_value()) {
      ...
      for (const auto &resource : resources) {
        std::cout << "  " << resource.name << " : " << resource.type
                  << std::endl;
        ...
        if (resource.descriptor_set.has_value()) {
          if (!bindingInfo.empty())
            bindingInfo += ", ";
          bindingInfo +=
              "set=" + std::to_string(resource.descriptor_set.value());
        }
        if (resource.location.has_value()) {
          if (!bindingInfo.empty())
            bindingInfo += ", ";
          bindingInfo +=
              "location=" + std::to_string(resource.location.value());
        }
        ...
        for (const auto &decoration : resource.decorations) {
          if (decoration.decoration != SpvDecoration::Binding &&
            ...
            std::string decorationStr =
                decorationToString(decoration.decoration);
            if (!decoration.params.empty()) {
              decorationStr += "(";
